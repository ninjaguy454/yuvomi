...
for(const baseVersion of [10038,10039])test(`encrypted populated ${baseVersion} -> 10040 preserves household and Rotation evidence; restart does not replay`,()=>{
  ...
  try {
    ...
    const boot=()=>{
      ...
      assert.match(result.stdout+result.stderr,/SCHEMA 10040/);
      return [...(result.stdout+result.stderr).matchAll(/Migration (\d+) applied:/g)].map(match=>Number(match[1]));
    };
    assert.deepEqual(boot(),baseVersion===10038?[10039,10040]:[10040]);
    ...
    assert.equal(afterHistory.length,before.tables.schema_migrations.rows.length+(baseVersion===10038?2:1));
    ...
    assert.equal(d.prepare('SELECT max(version) v FROM schema_migrations').get().v,10040);
