...
async function refreshSavedGoogleIdentity(database, place, userId) {
  ...
  if (duplicate) {
    throw new PlaceProviderError('This Google identity now belongs to another saved Vidamia Place. An administrator needs to reconcile the two saved Places.', { status: 409, code: 'place_identity_conflict' });
  }
  database.prepare(`
  ...
  try {
    ...
    if (existing) {
      return res.status(409).json({
        error: 'That Google place is already saved in Vidamia Places.',
        code: 409,
