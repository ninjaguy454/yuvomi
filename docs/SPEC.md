...
| code_hash | TEXT | NOT NULL. SHA-256, deliberately not bcrypt: the code is not a password but a ~49-bit random value from Ordoma's own generator. Key stretching protects weak secrets; here there is nothing weak to protect, while ten bcrypt runs per sign-in attempt would make the sign-in itself the target |
...
The flag is copied onto the follow-up instance; without that the series would fall back to the due-date grid from its second run on, and it would do so silently, because the follow-up looks complete either way. The completion day is read in the household's own zone (`TZ`, see `server/utils/timezone.js`): ticking a task off at 00:30 must count as the new day, or a weekly task would come back six days later. The anchor is local to Ordoma and does not travel over CalDAV, because RFC 5545 has no way to express it and a mirrored VTODO carries the rule alone. The shared calculation lives in `nextDueAfterCompletion()` in `server/services/recurrence.js`, deliberately separate from the route because resettable countdowns want the same "counts from the moment you touched it" arithmetic (#647).
...
- **CalDAV:** mirrored from `CATEGORIES` on VTODO objects and pushed back. `CATEGORIES` is only in the managed property set because Ordoma holds the **complete** list — mirroring a single value would make every push delete the tags the server knows and Ordoma never saw.
...
longer returns, so removing an event in iCloud/Nextcloud also removes it in Ordoma. Deletion is scoped
...
automatically into that one calendar — the recommended setup is a dedicated "Ordoma" calendar in
...
| outlook_change_key | TEXT | Graph ETag after Ordoma's last own write, nullable — the basis of drift reconciliation |
...
back to the Ordoma state; an event missing from the listing was deleted in Outlook and is
...
makes "Ordoma is the source of truth" hold against remote edits at every interval, at the cost of
roughly one extra request per calendar per run.

### Calendar Pending Deletions
Tombstones for events deleted in Ordoma that still exist at the provider (migration v103, #593).
...
- A change is a **patch of the original object**, never a rebuild: only the mirrored properties
  ...
  visibility and subtasks are Ordoma-internal and never trigger a push.
...
- **Creating: outbound since migration v136 (#695).** Until then this was out of scope, on the
  ...
  directions" the whole time. A task created in Ordoma now names its destination in
...
- **Lossy mappings are held from both ends.** Ordoma has four priority levels and three statuses,
...
Ordoma now keeps the structured `N` components (family, given, additional, prefix, suffix) and
...
**Webhook payload template.** An empty template sends Ordoma's own body shape, which suits receivers
...
into the budget because it is being paid now, whereas these were paid before Ordoma existed and never
...
`DB_ENCRYPTION_KEY` (SQLCipher) is strongly recommended. Ordoma is **not** a medical device and
...
- Prefer `/api.js` for Ordoma REST calls. It prefixes `/api/v1`, sends session credentials, refreshes CSRF tokens, and uses non-cached fetches for user data.
