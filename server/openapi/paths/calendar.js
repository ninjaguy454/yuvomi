  ...
  return {
    '/api/v1/calendar': {
      ...
      post: op({
        ...
        tag: 'Calendar',
        stateChanging: true,
        description: 'Supports optional document-storage attachments via `attachment_name`, `attachment_mime`, `attachment_size`, and `attachment_data` (base64 data URL). New attachments are linked through `attachment_document_id`; legacy events may still return `attachment_data`. `place_id` may reference an active immutable Vidamia Place while `location` remains editable display text. `event_kind: travel` plus `travel_details` explicitly creates or links a travel planning context for the assigned travelers; ordinary located Events remain advisory and never infer travel. Omit `travel_details.planning_context_id` to create a distinct trip, or provide an existing travel context ID when this Event is a related segment of that same trip. Distinct overlapping contexts create an unresolved keep-one conflict and are never combined automatically. Set `target_caldav_account_id` and `target_caldav_calendar_url` to push the event to a CalDAV calendar (omit or null for a local-only event).',
    ...
    '/api/v1/calendar/{id}': {
      ...
      put: op({
        ...
        params: [idParam()],
        stateChanging: true,
        description: 'Supports document-storage attachments. Omit attachment fields to preserve the current attachment, send new `attachment_data` to create and link a document, or set `remove_attachment` to true to unlink it without deleting the library document. Legacy events may still return `attachment_data`. Omit `place_id` to preserve the Vidamia Place reference, set it to null to detach it, or send an active Place ID; renaming that Place does not change Event identity. `event_kind: travel` updates the explicit travel projection and shared planning context; `travel_details.planning_context_id` explicitly moves the Event to an existing related travel context, while changing the kind back to `general` removes only that projection. Distinct overlapping contexts remain separate and require an explicit keep-first or keep-second conflict resolution. Changing a mirrored field (title, description, location, color, all-day, start/end, recurrence) of an event synced to Google pushes the change there, and switching `target_google_calendar_id` moves it to the other Google calendar. The remote call runs after the response and is retried by the next sync run if it fails.',
        requestBody: jsonBody(null),
        responses: {
          200: {
