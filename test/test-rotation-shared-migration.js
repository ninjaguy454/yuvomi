...
for(const baseVersion of [10038,10039])test(`encrypted populated ${baseVersion} -> 10043 preserves household and Rotation evidence; restart does not replay`,()=>{
  ...
  try {
    ...
    const boot=()=>{
      ...
      assert.match(result.stdout+result.stderr,/SCHEMA 10043/);
      return [...(result.stdout+result.stderr).matchAll(/Migration (\d+) applied:/g)].map(match=>Number(match[1]));
    };
    assert.deepEqual(boot(),baseVersion===10038?[10039,10040,10041,10042,10043]:[10040,10041,10042,10043]);
    ...
    assert.equal(afterHistory.length,before.tables.schema_migrations.rows.length+(baseVersion===10038?5:4));
    ...
    assert.equal(d.prepare('SELECT max(version) v FROM schema_migrations').get().v,10043);
