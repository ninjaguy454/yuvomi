...
- **Fixed** preserves Vidamia's existing assignment behavior.
...
Reopening a task from the completed source cycle removes the entire generated next cycle only when all of those generated tasks are still safe to discard. If any generated task contains work under Vidamia's existing recurrence safety checks, none of the group is deleted.
