...
function row(binding, groups = [], { skills = [], places = [] } = {}) {
...
export function renderRotationBindings(value = [], config = {}) {
  return `<section data-rotation-bindings><h3>Rotation</h3><p class="form-hint">Reuse an ordered household group. Each Activity maintains its own rotation; this Activity's subtasks share one result.</p>
...
export function bindRotationBindings(container, { skills = [], places = [], readOnly = false } = {}) {
  ...
  root.addEventListener('click', event => {
    ...
    if (event.target.closest('[data-rotation-add]')) {
      ...
      root.querySelector('[data-rotation-rows]').insertAdjacentHTML('beforeend', row({purpose_key:key}, groups, {skills,places}));
...
export function renderRotationContext(contexts = [], { compact = false } = {}) {
  ...
  return `<div class="task-rotation-context" data-task-rotation-context>${contexts.map(({purpose_key,label: purposeLabel,occurrence,position,reason}) => {
    ...
    return `<div><strong>${esc(label)}</strong>${position ? ` <span class="badge">${ordinal(position)}</span>` : ''}${compact
      ...
      : `<ol>${occurrence.order.map(member => `<li>${esc(member.display_name)}</li>`).join('')}</ol>`}</div>`;
  }).join('')}</div>`;
}
