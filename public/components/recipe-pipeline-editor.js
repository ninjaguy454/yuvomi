...
export function openRecipePipeline(initialRecipe, { onSaved = () => {}, onDuplicate = () => {} } = {}) {
  ...
  function render() {
    ...
    if (busy && !recipe.pipeline_current_source_hash) content.replaceChildren();
    ...
    else if (editing && draft) setHTML(content, editorHTML(draft, selectedId, recipe, division));
    else if (graph) {
      ...
      setHTML(content, `<div class="pipeline-empty"><h3>This recipe is managed by its provider</h3><p>Create an editable copy in Yuvomi to build a pipeline. Provider recipes stay unchanged.</p><button type="button" class="btn btn--secondary" data-command="duplicate" ${busy ? 'disabled' : ''}>Duplicate as native recipe</button></div>`);
    } else {
      setHTML(content, `<div class="pipeline-empty"><h3>Build your Cooking Map</h3><p>Choose each operation’s ingredients, ready states and outputs. Yuvomi draws the connections from those selections.</p><p>${recipe.pipeline_invalid ? 'The saved pipeline could not be read. Building and saving a replacement requires your review.' : 'There is no structured pipeline yet. Nothing is inferred from the written instructions.'}</p>${!editable && !busy ? '<p>Only the recipe’s creator can edit its pipeline.</p>' : ''}</div>${writtenRecipe(recipe)}`);
    }
  ...
  openModal({ title: `${recipe.title} · Cooking Map`, size: 'xl', initialFocus: 'heading',
    ...
    onClose: () => { closed = true; graphCleanup(); },
    onSave(mounted) {
