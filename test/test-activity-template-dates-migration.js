...
process.env.SESSION_SECRET = 'template-dates-migration-test';
const { ALL_MIGRATIONS } = await import('../server/db.js');

test('10036 production-shaped templates migrate additively to10037 once and restart without replay', () => {
  const directory = mkdtempSync(path.join(tmpdir(), 'activity-template-dates-'));
  ...
  try {
    ...
    const original = d.prepare('SELECT * FROM activity_templates WHERE id=1').get();
    ...
    const after = migrated.prepare('SELECT * FROM activity_templates WHERE id=1').get();
    for (const [key, value] of Object.entries(original)) assert.deepEqual(after[key], value, key);
    assert.equal(after.start_date, null); assert.equal(after.due_date, null);
    assert.deepEqual(migrated.prepare('SELECT * FROM tasks WHERE id=1').get(), taskBefore);
    ...
    assert.ok(path.basename(directory).startsWith('activity-template-dates-'));
