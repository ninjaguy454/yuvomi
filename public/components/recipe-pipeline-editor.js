...
import { newOperation, addOutput, removeOutput, removeOperation, moveOperation, divideResource, bindIngredient } from '/utils/recipe-pipeline-edit.js';
...
export function operationFields(document, operationId) {
  ...
  return `<div class="pipeline-fields">
    ...
    <fieldset class="pipeline-options"><legend>Requires · ready states</legend><p class="pipeline-help">For example, a preheated oven or a prepared pan. Add an operation that produces a ready state to make it available here.</p><div class="pipeline-checks">${inputs('readiness', 'requires') || '<p>No ready states defined yet.</p>'}</div></fieldset>
...
function editorHTML(document, selectedId, recipe) {
  ...
  return `<div class="pipeline-editor"><aside class="pipeline-operation-picker" aria-label="Operations">
    ...
    <ol>${document.operations.map(operation => `<li><button type="button" data-operation="${esc(operation.id)}" aria-pressed="${operation.id === selectedId}">${esc(operation.label || 'Unnamed operation')}</button></li>`).join('')}</ol>
    <div class="pipeline-inline-actions"><button type="button" class="btn btn--secondary" data-command="up" ${index <= 0 ? 'disabled' : ''}>Move up</button><button type="button" class="btn btn--secondary" data-command="down" ${index < 0 || index === document.operations.length - 1 ? 'disabled' : ''}>Move down</button></div>
  ...
  <details class="pipeline-details"><summary>Divide an ingredient or component into portions</summary>
    ...
    <label class="form-group">Material to divide<select class="form-input" data-divide-resource><option value="">Choose an ingredient or component</option>${document.resources.filter(resource => resource.kind !== 'readiness').map(resource => `<option value="${esc(resource.id)}">${esc(resourceLabel(resource))}</option>`).join('')}</select></label>
    ${[1, 2].map(number => `<div class="pipeline-field-row"><label class="form-group">Portion ${number} name<input class="form-input" data-portion-name="${number}" maxlength="500"></label><label class="form-group">Portion ${number} quantity<input class="form-input" data-portion-quantity="${number}" maxlength="100" placeholder="Optional"></label></div>`).join('')}
...
export function openRecipePipeline(initialRecipe, { onSaved = () => {}, onDuplicate = () => {} } = {}) {
  ...
  function changed(next) {
    ...
    panel.querySelector('[data-pipeline-dirty]').value = JSON.stringify(draft);
  ...
  function render() {
    ...
    panel.querySelector('[data-pipeline-dirty]').value = dirty ? JSON.stringify(draft) : '';
    panel.querySelector('[data-pipeline-toolbar]').innerHTML = `<div class="pipeline-view-buttons" role="group" aria-label="Recipe view">${[['written', 'Written recipe'], ['pipeline', 'Pipeline']].map(([key, label]) => `<button type="button" class="btn btn--${view === key ? 'primary' : 'secondary'}" data-view="${key}" aria-pressed="${view === key}" ${busy ? 'disabled' : ''}>${label}</button>`).join('')}</div>
    ...
    if (busy && !recipe.pipeline_current_source_hash) content.innerHTML = '';
      ...
      catch (err) { content.innerHTML = '<p>The draft is not ready to draw. Edit its operations to resolve the issue below.</p>'; showError(err.message); }
    } else if (recipe.provider_account_id) {
      content.innerHTML = `<div class="pipeline-empty"><h3>This recipe is managed by its provider</h3><p>Create an editable copy in Yuvomi to build a pipeline. Provider recipes stay unchanged.</p><button type="button" class="btn btn--secondary" data-command="duplicate" ${busy ? 'disabled' : ''}>Duplicate as native recipe</button></div>`;
    } else {
      content.innerHTML = `<div class="pipeline-empty"><h3>Build your Cooking Map</h3><p>Choose each operation’s ingredients, ready states and outputs. Yuvomi draws the connections from those selections.</p><p>${recipe.pipeline_invalid ? 'The saved pipeline could not be read. Building and saving a replacement requires your review.' : 'There is no structured pipeline yet. Nothing is inferred from the written instructions.'}</p>${!editable && !busy ? '<p>Only the recipe’s creator can edit its pipeline.</p>' : ''}</div>${writtenRecipe(recipe)}`;
    }
    const footer = panel.querySelector('[data-pipeline-footer]');
    footer.hidden = !draft;
    footer.innerHTML = `<button type="button" class="btn btn--secondary" data-command="cancel" ${busy ? 'disabled' : ''}>Cancel</button><button type="button" class="btn btn--primary" data-command="save" ${busy ? 'disabled' : ''}>${busy ? 'Saving…' : 'Save pipeline'}</button>`;
    ...
    if (!dirty) refreshDirtySnapshot({ defer: false });
  }

  async function command(name, button) {
    if (busy) return;
    showError('');
    if (name === 'duplicate') { await onDuplicate(recipe); return; }
    if (name === 'edit') { if (editing) { editing = false; view = 'pipeline'; render(); } else startEditing(); return; }
    if (!draft) return;
    if (name === 'cancel') {
      if (dirty && !await confirmOverModal('Discard your unsaved pipeline changes?', { confirmLabel: 'Discard changes', danger: true, closeOnConfirm: false })) return;
      draft = null; dirty = false; editing = false; reviewConfirmed = false; message = ''; render(); return;
    }
    if (name === 'save') {
      ...
      try {
        ...
        onSaved(recipe);
      } catch (err) { if (panel.isConnected) showError(`${err.message} Your unsaved edits are still here.`); }
      finally { busy = false; if (panel.isConnected) render(); }
      return;
    }
    if (name === 'add') {
      ...
      if (!await confirmOverModal('Remove this operation and its unused outputs?', { confirmLabel: 'Remove operation', danger: true, closeOnConfirm: false })) return;
      selectedId = next.operations[0]?.id || null; changed(next); render();
    } else if (name === 'up' || name === 'down') { changed(moveOperation(draft, selectedId, name === 'up' ? -1 : 1)); render(); }
      ...
      const result = divideResource(draft, panel.querySelector('[data-divide-resource]').value, [1, 2].map(number => ({ name: panel.querySelector(`[data-portion-name="${number}"]`).value, quantity: panel.querySelector(`[data-portion-quantity="${number}"]`).value })));
      changed(result.document); selectOperation(result.operationId);
  ...
  openModal({ title: `${recipe.title} · Cooking Map`, size: 'xl', initialFocus: 'heading',
    ...
    onSave(mounted) {
      ...
      panel.addEventListener('input', event => {
        ...
        const target = event.target, data = target.dataset, next = copy(draft);
        ...
        if (data.operationField === 'label') {
          const label = panel.querySelector(`[data-operation="${selectedId}"]`);
      ...
      api.get(`/recipes/${recipe.id}`).then(response => {
        ...
        Object.assign(initialRecipe, response.data); recipe = initialRecipe;
        busy = false; render();
      }).catch(err => { if (panel.isConnected) { busy = false; recipe = { ...recipe, pipeline_can_edit: false }; render(); showError(`Could not load the latest recipe. Close this view and try again. ${err.message}`); } });
    },
  });
}
