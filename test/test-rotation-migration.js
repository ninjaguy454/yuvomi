...
test('encrypted production-shaped 10038 -> 10043 preserves all existing rows, indexes, triggers, foreign keys and restart history',()=>{
  ...
  try{
    ...
    const initial=boot();assert.deepEqual([...initial.matchAll(/Migration (\d+) applied:/g)].map(m=>Number(m[1])),[10039,10040,10041,10042,10043]);
    d=open();
    ...
    const afterHistory=d.prepare('SELECT * FROM schema_migrations ORDER BY version').all();assert.equal(afterHistory.length,history.length+5);d.close();
    assert.deepEqual([...boot().matchAll(/Migration (\d+) applied:/g)].map(m=>Number(m[1])),[]);
    d=open({readonly:true});assert.equal(d.prepare('SELECT max(version) v FROM schema_migrations').get().v,10043);assert.deepEqual(d.prepare('SELECT * FROM schema_migrations ORDER BY version').all(),afterHistory);
