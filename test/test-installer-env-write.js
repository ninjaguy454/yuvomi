...
test('renderEnvFile erzeugt KEY=value-Zeilen mit Header und Schluss-Newline', () => {
  ...
  assert.match(out, /^# Generated by Vidamia Installer\n/);
...
test('install.sh übernimmt einen bestehenden Schlüssel aus der .env', (t) => {
  // Use an installed Git Bash on Windows rather than the optional WSL shim.
  const bash = process.platform === 'win32'
    ? [process.env.ProgramFiles, process.env['ProgramFiles(x86)']].filter(Boolean)
      .map(directory => join(directory, 'Git', 'bin', 'bash.exe')).find(existsSync)
    : 'bash';
  if (!bash) {
    t.skip('No Git Bash is installed to exercise the isolated Unix helper on Windows');
    return;
  }
  ...
  const dir = mkdtempSync(join(tmpdir(), 'oikos-env-'));
  try {
    // A file preserves shell quoting through Windows process argument parsing.
    writeFileSync(join(dir, 'read-helper.sh'), `${fn.replace(/\r\n/g, '\n')}\nread_existing_env_value "$1" || printf '<none>'\n`);
    const run = key => execFileSync(bash, ['--noprofile', '--norc', './read-helper.sh', key],
