...
export function mealsPaths() {
  return {
    '/api/v1/meals': {
      get: op({ summary: 'List meal plan entries', tag: 'Meals', description: 'Returns the selected `dish`, current `menu_items` (one entree and up to three sides), ingredients, participant roles/statuses, exact planned portions, rounded cook portions, Place, timing window, Calendar conflicts and execution summary. Legacy recurring and skipped records remain readable.' }),
    ...
    '/api/v1/meals/plan-defaults': {
      get: op({ summary: 'Get household Meal Plan Default Settings', tag: 'Meals', description: 'Returns chooser failsafe defaults, safe role defaults (`round_robin` Cook and opt-in Supervisor), and nested `grocery_settings`. Grocery settings include the effective default Shopping list, alphabetically ordered choices, create-list state, grouping preference, and weekly draft/finalization weekday and local time (Monday = 0).' }),
      put: op({ summary: 'Update household Meal Plan Default Settings', tag: 'Meals', admin: true, stateChanging: true, requestBody: jsonBody(null, 'Body: { chooser_terminal_strategy?, chooser_terminal_user_id?, chooser_round_robin_user_ids?: number[], default_cook_strategy?: "none"|"round_robin", default_supervisor_strategy?: "none"|"round_robin", grocery_settings?: GrocerySettingsInput }'), description: 'Updates defaults for future occurrences and may update the nested weekly grocery settings in the same request. Explicit per-rule Cook/Supervisor choices remain authoritative. Existing Meal history is never rewritten.' }),
    },
    '/api/v1/meals/grocery-settings': {
      get: op({ summary: 'Get household Meal grocery settings', tag: 'Meals', description: 'Compatibility endpoint for the grocery settings also returned by Meal Plan defaults. Returns Shopping lists alphabetically, the configured and effective default list, `shopping_list_action: "create"` when none exists, grouping availability, and the weekly cutoff model. `weekly_timing.automatic_execution` remains false until a clock-driven scheduler is implemented.' }),
      put: op({ summary: 'Update household Meal grocery settings', tag: 'Meals', admin: true, stateChanging: true, requestBody: jsonBody(null, 'Body: { enabled?, default_shopping_list_id?, new_shopping_list_name?, auto_create_grocery_draft?, auto_finalize_grocery?, timing_mode?: "weekly"|"per_meal", draft_weekday?: 0..6, draft_time?: "HH:MM", finalization_weekday?: 0..6, finalization_time?: "HH:MM", grouping_mode?: "ingredient"|"category"|"meal"|"recipe", grocery_lead_minutes? }'), description: 'When no explicit default exists, the first Shopping list alphabetically is effective. `new_shopping_list_name` creates and selects a list atomically. Weekly cutoffs describe the planning week before the target Meal week and take conceptual precedence over legacy per-Meal lead minutes. Automatic clock-driven execution of those cutoffs is not provided by this endpoint; manual preparation retains existing behavior.' }),
    },
    '/api/v1/meals/{id}/decisions': {
      post: op({ summary: 'Save a member Meal participation, portion and food decision', tag: 'Meals', params: [idParam()], stateChanging: true, requestBody: jsonBody(null, 'Body supports participation, portion_amount, expected_revision, choice_kind, menu_item_ids, selected_recipe_id, selected_meal_title, notes, confirmed, beneficiary_user_id, device_key, and notify_on_menu_change. The menu-change opt-in is retained only while participation is not_participating.'), description: 'The portion amount (0.01..1000, at most two decimals) is saved per occurrence and beneficiary and survives opting out. Omitted amounts retain the saved value or default to 1. expected_revision rejects stale edits with 409. Exact requested totals count each opted-in member once; cooking amounts round up per selected dish, preserving explicit fixed cook targets. Recipe yields are optional: explicit yields scale ingredients by cooked portions divided by yield; unset yields retain legacy quantity behavior. Selected sides and individual Meals use the existing canonical grocery ledger. Participation is independent from chooser responsibility. For fixed/round-robin slots, only the active participating chooser may submit entrée/side IDs from the current menu generation; released-generation IDs return 409 and cannot fulfill the new chooser obligation. Confirmation publishes the current generation but does not lock later editing before closeout. Other participants submit household with zero menu IDs or an individual backup with zero menu IDs plus selected_recipe_id and/or selected_meal_title. A person choosing not_participating may set notify_on_menu_change to receive an existing-pipeline Meal reminder when a later published shared menu actually changes. Backup materializes a linked per-person Meal. Personal Choice rejects shared/backup choices and accepts personal, restaurant, or takeout. Administrators explicitly act for a beneficiary; beneficiary, actor and optional device_key are audited. A changed or superseded chooser responsibility preserves the last fulfilled menu while releasing only an unpublished draft and opening a blank replacement generation.' }),
    },
