# Vidamia installation

Vidamia is an independent household-management platform derived from open-source Yuvomi. Existing technical names, paths and environment variables remain compatible.

> **Upstream packaging reference.** The clone URLs, published images and app-store instructions in this inherited guide refer to upstream Yuvomi. They do not identify a published Vidamia image. Use the validated image built from this fork for Vidamia, with the existing data volumes and configuration. Do not switch a running Vidamia instance to an upstream image as part of a branding change.

## Quick Install

Three ways to get Vidamia running from scratch:
...
Complete setup instructions for Vidamia - from Docker installation to your first login.
...
For local-only access, the Docker container is all you need. If you want to access Vidamia from other devices on your network or the internet, add Nginx as a reverse proxy with SSL.
...
Docker. Vidamia supports Podman out of the box: both installers auto-detect it, and a
...
There are seven ways to get Vidamia running. **Option A** (web installer) is recommended for most users — it walks you through every step in your browser. **Option B** (pre-built image) is a quick manual alternative. **Option C** (build from source) is for contributors or custom builds. **Options D–F** install directly from a NAS/home-server app store with no terminal required: **Option D** (TrueNAS SCALE), **Option E** (Umbrel), and **Option F** (Unraid). **Option G** covers Portainer, whether you paste the stack or let it follow this repository via Git.
...
- Advanced settings — Single Sign-On (OIDC), the three home-network permissions (they lift the SSRF protection and are asked as one group), the calendar sync interval, live currency rates and the Web-Push contact. Everything that decides what Vidamia connects to
...
The final screen also links to the next three steps on your new instance: inviting your family, choosing which modules to enable, and installing Vidamia on your phones. Running the wizard again on an installation that already has an admin account is a supported case — it writes your `.env`, restarts the container and takes you to that same screen instead of failing.
...
On the first visit, Vidamia detects that no account exists yet and guides you through
...
After you submit, Vidamia creates the admin, signs you in automatically, and the setup
...
No terminal required. Upstream Yuvomi is available directly in the TrueNAS SCALE Community Apps Catalog. This is an upstream packaging reference, not a Vidamia release channel.
...
No terminal required. Upstream Yuvomi is available in the Umbrel App Store — everything runs on, and stays on, your Umbrel. This is an upstream packaging reference, not a Vidamia release channel.
...
No terminal required. Upstream Yuvomi ships as an Unraid Community Applications template. This is an upstream packaging reference, not a Vidamia release channel.
...
| `OIKOS_HTTP_PORT` | Host port that the compose file maps to the container's port 3000. Change this to expose Vidamia on a different host port; the app inside the container always listens on 3000. | `3000` | No |
...
page. These channels are configured in the UI and do not require environment variables. The Vidamia
...
- **The app must be installed to the Home Screen.** iOS delivers Web Push only to installed
  home-screen web apps, never to a Safari tab. Open Vidamia in Safari, then Share ->
  "Add to Home Screen".
...
- **Check iOS Settings -> Notifications -> Vidamia**: "Allow Notifications" must be on, and a Focus
...
- **The VAPID subject must be routable.** Apple validates the contact URI in the signed token and
  ...
  to work. Vidamia derives a usable value from the SMTP sender address or `BASE_URL`; set
...
| `EMAIL_FROM_NAME` | Sender display name. | `Vidamia` | No |
...
\* Not required to start Vidamia. Without it (or without SMTP configured) the self-service reset
...
open an immediate preview. An optional album UUID limits the selection; otherwise Vidamia uses the
...
Uploads fail closed: if WebDAV cannot accept the file, Vidamia rejects the upload instead of silently
...
below. Vidamia requests only `https://www.googleapis.com/auth/drive.file`; it cannot browse arbitrary
...
> **Access and backup boundary:** Vidamia visibility settings only control access through Vidamia.
...
> the same LAN or Docker network as Vidamia, so blocking private targets by default would break
...
events created, edited, deleted, or moved to another calendar in Vidamia are applied in Google as
...
Vidamia from changing anything in Google while still importing normally.
...
restricted to that API and to the server that runs Vidamia. This integration is independent of
Google Calendar OAuth. When it is not configured, saved and manual Vidamia Places continue to work
...
| `GOOGLE_PLACES_SEARCH_RADIUS_METERS` | Search bias radius around the selected Vidamia Place, up to 50 km | `50000` | No |
| `GOOGLE_PLACES_TIMEOUT_MS` | Provider request timeout, 1–30 seconds | `8000` | No |

Vidamia uses Text Search only after the user presses Search, requests at most ten results with a
fixed minimal field mask, never polls in the background, and keeps Google Place IDs separate from
Vidamia's own immutable Place IDs. Enable a billing account and **Places API (New)** only; Maps
...
budget alerts in addition to Vidamia's local safeguards. Google Maps navigation links do not
require the API key.

### Outlook Calendar Push - Microsoft Graph (Optional)

One-way push **Vidamia → Outlook.com** for personal Microsoft accounts (outlook.com, hotmail.com, M365 Family). Outlook.com does not support CalDAV, so this provider uses the Microsoft Graph API. Vidamia stays the source of truth: pushed events are created/updated/deleted in Outlook, and every sync run also checks the pushed events for remote drift (one cheap `changeKey` listing per calendar) — events edited in Outlook are reset to the Vidamia state, events deleted in Outlook are re-created. Multiple family accounts can be connected.
...
2. Name: e.g. `Vidamia Calendar Push`. Supported account types: **"Personal Microsoft accounts only"**.
...
Vidamia are applied on the server as well, and changes made there flow back. An outbound change is
attempted right when you save and retried by the next sync run if the server cannot be reached.
Editing preserves everything the server holds that Vidamia does not — attendees, alarms, categories
...
Nothing to configure — there is no environment variable, and Vidamia never reaches the network for
...
**Single sign-on does not skip it.** If you have a second factor set up, Vidamia asks for the code
...
Time matters: TOTP codes are derived from the clock, and Vidamia accepts a deviation of ±30 seconds.
...
Pocket ID documents Vidamia as one of its [client examples](https://pocket-id.org/docs/client-examples/yuvomi), which is a working set of values for the four variables below if you run that provider.
...
**Making SSO the only way in.** Even with SSO configured, Vidamia keeps a second door open: the login form stays, password reset stays, and every account carries a password hash. Set `AUTH_ALLOW_PASSWORD_LOGIN=false` and that door closes - the login page shows nothing but the SSO button, `POST /auth/login` is refused outright (the rule sits on the route, not just on the page), and password reset disappears with it rather than staying as a route that can still send mail.
...
> **Optional for local access, required for network/internet access.** If you only access Vidamia on the same machine (localhost), you can skip this section.

When exposing Vidamia to your local network or the internet, you need HTTPS for security. Nginx acts as a reverse proxy that handles SSL termination and forwards requests to the Docker container.
...
Vidamia ships with an example configuration. Copy it and replace `deine-domain.de` with
...
On RHEL-based systems you can run Vidamia as a rootless systemd service via Podman
[Quadlet](https://docs.podman.io/en/latest/markdown/podman-systemd.unit.5.html). Vidamia
...
The restore helper validates that the file is a Vidamia database before replacing the active database. It also keeps a pre-restore copy next to the database file for emergency rollback.
...
target" field, and a task created in Vidamia with a target set is uploaded on the next run (or right
...
to everyone but its creator and admins inside Vidamia, but an **inbound sync still rewrites its
...
account. If you want your Apple tasks in Vidamia, keep them in a CalDAV-backed list (Nextcloud,
...
reverse — deleting an event in Vidamia removes it from Outlook on the *next* run, not instantly.

Editing a pushed event in Outlook is pointless: Vidamia is the source of truth and resets it to its
own state on the next run, and re-creates it if you delete it there. To get rid of an event for
good, delete it in Vidamia. Note also that disconnecting an account leaves everything already
pushed behind in Outlook — clear those events in Vidamia *before* disconnecting, or delete them by
...
Before that, Vidamia discarded any calendar object whose URL did not contain `.ics`. That extension
