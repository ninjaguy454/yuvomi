 ...
 */
export function singleEmailAddress(value) {
  ...
  const domain = domainToASCII(raw.slice(at + 1));
  if (at > 64 || /[:]/.test(local) || local.startsWith('.') || local.endsWith('.') || local.includes('..')) return null;
...
export function memberEmail(userId, { db } = {}) {
  ...
  const row = database.prepare(`
    ...
    LIMIT 1
  `).get(userId);
  return singleEmailAddress(row?.email);
}
...
export function listEmailableMembers({ db } = {}) {
  ...
  `).all()
    .map((row) => ({ ...row, email: singleEmailAddress(row.email) }))
