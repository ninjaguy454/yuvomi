...
// --------------------------------------------------------
// Participants, progress, and location
// --------------------------------------------------------

/** Normalize the several Task API user shapes into one stable UI shape. */
export function normalizeParticipant(user, role = 'participant') {
  ...
  return {
    ...
    family_role: user.family_role || null,
...
/** One person once, even when assignment and responsibility rows both name them. */
...
/** Resolve a first-class subtask's assignees without relying on Tasks-page state. */
export function subtaskParticipants(subtask, users = []) {
  if (subtask?.assigned_users?.length) {
    return subtask.assigned_users
      .map((user) => normalizeParticipant(user, 'assignee'))
      .filter(Boolean);
  }
  if (!subtask?.assigned_to) return [];
  const known = users.find((user) => Number(user.id) === Number(subtask.assigned_to));
...
/** Human-readable location used by sorting, grouping, cards, and Task Details. */
