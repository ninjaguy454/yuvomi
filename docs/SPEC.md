...
| code_hash | TEXT | NOT NULL. SHA-256, deliberately not bcrypt: the code is not a password but a ~49-bit random value from Vidamia's own generator. Key stretching protects weak secrets; here there is nothing weak to protect, while ten bcrypt runs per sign-in attempt would make the sign-in itself the target |
...
| status | TEXT | open, in_progress, done — the CHECK still permits the retired `archived` value, but nothing writes it since migration v132 |
...
The flag is copied onto the follow-up instance; without that the series would fall back to the due-date grid from its second run on, and it would do so silently, because the follow-up looks complete either way. The completion day is read in the household's own zone (`TZ`, see `server/utils/timezone.js`): ticking a task off at 00:30 must count as the new day, or a weekly task would come back six days later. The anchor is local to Vidamia and does not travel over CalDAV, because RFC 5545 has no way to express it and a mirrored VTODO carries the rule alone. The shared calculation lives in `nextDueAfterCompletion()` in `server/services/recurrence.js`, deliberately separate from the route because resettable countdowns want the same "counts from the moment you touched it" arithmetic (#647).
...
- **CalDAV:** mirrored from `CATEGORIES` on VTODO objects and pushed back. `CATEGORIES` is only in the managed property set because Vidamia holds the **complete** list — mirroring a single value would make every push delete the tags the server knows and Vidamia never saw.
...
| recurrence_rule | TEXT | iCal RRULE — supported subset `FREQ` (DAILY/WEEKLY/MONTHLY/YEARLY), `INTERVAL`, `BYDAY`, and a mutually-exclusive end condition `UNTIL` **or** `COUNT`. Stored in **both spellings**: locally created series hold the bare rule body, series read in from CalDAV/ICS hold the full line including its `RRULE:` prefix. Unifying them would cost more than a migration is worth, so every *output* path has to resolve the ambiguity instead — through `rruleValue()` (bare value, for APIs that want the value) and `rruleLine()` (the ICS line with exactly one prefix), both in `server/services/recurrence.js` since **v2.12.0** (#761). Until then each of six modules built the line for itself, five correctly and one not: the export feed put a prefix in front of a rule that already had one and emitted `RRULE:RRULE:FREQ=...`, which Apple tolerates and strict parsers reject. An unchanged rule is written back verbatim rather than rebuilt from the form, so parts outside the supported subset (`WKST`, `BYMONTHDAY`, `BYSETPOS`) survive an edit to any other field (v2.8.1, #756) |
...
longer returns, so removing an event in iCloud/Nextcloud also removes it in Vidamia. Deletion is scoped
...
automatically into that one calendar — the recommended setup is a dedicated "Vidamia" calendar in
...
| outlook_change_key | TEXT | Graph ETag after Vidamia's last own write, nullable — the basis of drift reconciliation |
...
back to the Vidamia state; an event missing from the listing was deleted in Outlook and is
...
makes "Vidamia is the source of truth" hold against remote edits at every interval, at the cost of
roughly one extra request per calendar per run.

### Calendar Pending Deletions
Tombstones for events deleted in Vidamia that still exist at the provider (migration v103, #593).
...
- A change is a **patch of the original object**, never a rebuild: only the mirrored properties
  ...
  visibility and subtasks are Vidamia-internal and never trigger a push.
...
- **Creating: outbound since migration v136 (#695).** Until then this was out of scope, on the
  ...
  directions" the whole time. A task created in Vidamia now names its destination in
...
- **Lossy mappings are held from both ends.** Vidamia has four priority levels and three statuses,
...
Vidamia now keeps the structured `N` components (family, given, additional, prefix, suffix) and
...
**Webhook payload template.** An empty template sends Vidamia's own body shape, which suits receivers
...
into the budget because it is being paid now, whereas these were paid before Vidamia existed and never
...
`DB_ENCRYPTION_KEY` (SQLCipher) is strongly recommended. Vidamia is **not** a medical device and
...
- The screensaver is unchanged and lays itself over the wall mode after its idle time, exactly as it does over the normal dashboard.
...
Module for managing household staff workflows. Shared navigation and controls follow the selected theme; module identity colors remain separate.
...
- Prefer `/api.js` for Vidamia REST calls. It prefixes `/api/v1`, sends session credentials, refreshes CSRF tokens, and uses non-cached fetches for user data.
...
**The stage follows the selected theme.** Warm uses ivory/taupe neutrals, Neutral
preserves the established surface hierarchy, and Cool uses slate-tinted neutrals.
Each supports Light, Dark and System appearance independently. The app ground
uses `--color-bg`; cards, cells and work areas use `--color-surface` and its
work/raised variants. One inset-well fill (`--color-surface-3`) is the only permitted tint
...
**Shared actions and selection use the chosen theme's accent** (`--color-accent`):
bronze for Warm, graphite for Neutral, slate blue for Cool. Hover, selected
surfaces, focus rings and contrasting ink use the same token family in Light
and Dark; `--color-accent-light` provides its tinted surface. Module/data colors
and recognizable status colors remain separate. Historical upstream violet
measurements describe the old palette; current checks measure the real rendered
control foreground/background pairs for every theme.
