...
export function googlePlacesStatus(database = null, userId = null, now = new Date()) {
  ...
  return {
    ...
    privacy_notice: 'Search terms and the selected origin are sent to Google Places through this Vidamia server.',
  };
}
...
export async function searchGooglePlaces(database, {
  ...
  if (!config.configured) {
    throw new PlaceProviderError('Google Places is not configured for this Vidamia instance.', {
      status: 503,
