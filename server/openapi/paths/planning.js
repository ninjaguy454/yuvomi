  ...
  const paths = {
    ...
    '/api/v1/planning/place-search': {
      post: op({
        ...
        stateChanging: true,
        requestBody: jsonBody(null),
        description: 'Body: { query, origin_place_id?, origin_text?, included_type? }. Search is request-controlled and returns transient Google results keyed by external_place_id; it does not create Vidamia Places.',
      }),
    },
    ...
    '/api/v1/planning/admin/places': {
      post: op({ summary: 'Create a user-maintained Vidamia Place', tag: 'Planning', admin: true, stateChanging: true, requestBody: jsonBody(null), description: 'Creates a reusable Place with immutable Vidamia identity, optional parent Place, address and coordinates. Home Places do not require an address.' }),
    },
    '/api/v1/planning/admin/places/from-google': {
      post: op({
        summary: 'Save a Google result as a reusable Vidamia Place',
        tag: 'Planning',
        admin: true,
        stateChanging: true,
        requestBody: jsonBody(null),
        description: 'Stores Google external_place_id separately from the immutable Vidamia Place ID. Name and address remain user-maintained fields; a duplicate provider identity returns 409.',
      }),
    },
    '/api/v1/planning/admin/places/{id}': {
      put: op({ summary: 'Update a Vidamia Place without changing its identity', tag: 'Planning', admin: true, params: [idParam()], stateChanging: true, requestBody: jsonBody(null) }),
      delete: op({ summary: 'Delete an unused Vidamia Place', tag: 'Planning', admin: true, params: [idParam()], stateChanging: true, description: 'Returns 409 while schedules, templates, Tasks or child Places still reference the Place. Calendar Event and Meal links use their schema deletion policy and do not justify deleting other guarded references.' }),
    },
    '/api/v1/planning/admin/places/{id}/refresh-external-id': {
      post: op({ summary: 'Refresh a saved Google Place identity', tag: 'Planning', admin: true, params: [idParam()], stateChanging: true, description: 'Refreshes a stale Google place ID without changing the immutable Vidamia Place ID; a provider-ID collision returns 409.' }),
    },
