...
On a runtime with Rotation Groups, `ROTATION=1` attaches a shared Rotation purpose to each synthetic routine before opening it. This checks the same optimistic interaction while Rotation context is displayed; generated Group IDs are recorded with the fixture evidence. It does not change the default probe or require Rotation support when comparing an older baseline.
